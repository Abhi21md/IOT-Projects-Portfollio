...
void voteRecordedTone() {
  tone(BUZZER, 1500, 120);
  delay(120);
  tone(BUZZER, 2000, 120);
}
void alreadyVotedTone() {
  tone(BUZZER, 500, 200);
  delay(100);
  tone(BUZZER, 400, 200);
}
...
void setup() {
  ...
  Firebase.getString(firebaseData, "/VotedVoters") ? votedVoters = firebaseData.stringData() : votedVoters = "";

  Firebase.getString(firebaseData, "/DuplicateVoters") ? duplicateVoters = firebaseData.stringData() : duplicateVoters = "";
...
void handleVoter(int voterID) {
  ...
  while (vote == 0 && !supervisorLock) {
    if (digitalRead(BUTTON_BJP) == LOW)
      vote = 'B';
    else if (digitalRead(BUTTON_CONGRESS) == LOW)
      vote = 'C';
    else if (digitalRead(BUTTON_JDS) == LOW)
      vote = 'J';
    else if (digitalRead(BUTTON_NOTA) == LOW)
      vote = 'N';
    delay(40);
  }
  ...
  if (vote == 'B')
    showVoteConfirmation("BJP"), bjpVotes++;
  else if (vote == 'C')
    showVoteConfirmation("CONGRESS"), congressVotes++;
  else if (vote == 'J')
    showVoteConfirmation("JDS"), jdsVotes++;
  else if (vote == 'N')
    showVoteConfirmation("NOTA"), notaVotes++;
